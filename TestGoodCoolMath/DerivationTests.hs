...
spec = do
  describe "Derivation" $ do
    ...
    it "should return correct derivative of a fraction" $ property $
      \ dVar num den -> der dVar (Frac num den) =->..<-= Frac
        ( (Sum . fromList) [(Prod . fromList) [den, der dVar num], Neg ((Prod . fromList) [num, der dVar den])] )
        ( (Prod . fromList) [den, den] )
    it "should return the correct derivative of an exponential" $ property $
      \ dVar e -> der dVar (Exp e) =->..<-= (Prod . fromList) [e, Exp ((Sum . fromList) [e, Neg (IntLit 1)]), der dVar e]
